...
class Model():
    ...
    def __init__(self, train_input_file, train_target_file,
            ...
            save_step = 10, eval_step = 1000,
            param_histogram=False, restore_model=True,
        ...
        if init_train:
            self.train_reader = myreader.SeqReader(train_input_file,
                    train_target_file, vocab_file, batch_size)
            self.train_reader.start()
            self.train_data = self.train_reader.read()
            self.eval_reader = myreader.SeqReader(test_input_file, test_target_file,
                    vocab_file, batch_size)
            self.eval_reader.start()
            self.eval_data = self.eval_reader.read()

        self.model_file = output_dir + '/model.ckpl'  #infer  use
        # self.model_file = path.join(output_dir, 'model.ckpl') #train use
        # self.model_file = tf.train.latest_checkpoint(output_dir)
        ...
        if init_infer:
            self.infer_vocabs = myreader.read_vocab(vocab_file)
    ...
    def train(self, epochs, start=0):  #带默认值的参数放在最后
        if not self.init_train:
            raise Exception('Train graph is not inited!')
        with self.train_graph.as_default():
            if path.isfile(str(self.model_file) + '.meta') and self.restore_model:
            ...
            for step in range(start, epochs):
                ...
                if step % self.save_step == 0:
                    ...
                    input_text = myreader.decode_text(in_seq[sid],
                        self.eval_reader.vocabs)
                    output_text = myreader.decode_text(output[sid],
                            self.train_reader.vocabs)
                    target_text = myreader.decode_text(target_seq[sid],
        ...
        with self.eval_graph.as_default():
            ...
            for step in range(0, self.eval_reader.data_size):
                ...
                for i in range(len(outputs)):
                    output = outputs[i]
                    target = target_seq[i]
                    output_text = myreader.decode_text(output,
                            self.eval_reader.vocabs).split(' ')
                    target_text = myreader.decode_text(target[1:],
                            self.eval_reader.vocabs).split(' ')
                    ...
                    if random.randint(1, prob) == 1:
                        print('====================')
                        input_text = myreader.decode_text(in_seq[i],
                                self.eval_reader.vocabs)
                        print('src:' + input_text)
    ...
    def infer(self, text):
        ...
        with self.infer_graph.as_default():
            in_seq = myreader.encode_text(text.split(' ') + ['</s>',],
            ...
            output = outputs[0]
            output_text = myreader.decode_text(output, self.infer_vocabs)
            return output_text
